...
- ✅ **Phase 3 완료** (일정편차 + 지연 알람): Config·Process별 일정편차(지연/선행/진행중/예정/계획준수)가 표에 표시됨. 지연일수(=실제 Output 최초 발생일 - Daily Plan 계획일, 완료목표일로 해석)를 계산해서 임계값(기본 주의 2일/위험 3일, 화면에서 조정 가능) 이상이면 알람 표시. 브라우저에서 주의 임계값을 1일로 낮추자 "+1일" 지연 셀들이 즉시 노란색으로 바뀌는 것을 확인. 자동 테스트 27/27 통과(합성 케이스로 지연/선행/진행중/예정 4가지 분기와 임계값 경계값 모두 검증). 코드 위치: `process-dashboard/lib/scheduleAnalysis.ts`, 테스트: `process-dashboard/tests/scheduleAnalysis.test.ts`.
- ⬜ Phase 4~6: 아직 시작 전.
