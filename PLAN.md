...
1. **시간 예산이 빠듯함** — Phase별로 다시 계산하니 합계 약 6.5~9시간으로, 6~8시간 예산의 상단이거나 넘을 수 있음. 범위를 줄일지, 시간을 더 확보할지, 되는 만큼만 하고 마무리할지부터 정해야 나머지 결정도 따라감.
2. **Waiver 미승인 물량을 출하 D-1 초안에 포함시킬지** — 원본 구현안(16장)에서 이미 "확정 필요" 업무 규칙으로 짚었던 항목인데 이번 PLAN.md엔 반영이 안 되어 있음. Phase 4(Risk 판정)와 Phase 5(D-1 초안) 둘 다에 직접 영향.
3. **수율 위험/주의 절대 임계값(90%/97%, 기준수율 대비 10%p/5%p)을 코드 상수로 고정해도 되는지** — Process별 목표 수율은 화면 입력으로 해결됐지만, 이 절대 임계값 자체는 여전히 코드 상수로 고정. 화면에서 조정 가능해야 하는지는 미정.
4. **Destination 우선순위가 "화면에서 자유 설정"으로 충분한지** — 회사에 이미 정해진 배정 우선순위 규칙이 있다면 자유 설정 UI 대신 그 규칙을 기본값으로 반영하는 게 맞을 수 있음.
